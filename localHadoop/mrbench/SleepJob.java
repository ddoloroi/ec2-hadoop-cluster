...
public class SleepJob {
  ...
  public static Job createJob(int numMapper, int numReducer,
                       long mapSleepTime, int mapSleepCount,
                       long reduceSleepTime, int reduceSleepCount, int deadline)
      throws IOException {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if(args.length < 1) {
      printUsage("number of arguments must be > 0");
    }
    ...
    for(int i=0; i < args.length; i++ ) {
      if(args[i].equals("-m")) {
        ...
        if (numMapper < 0) {
          printUsage(numMapper + ": numMapper must be >= 0");
        }
      }
      else if(args[i].equals("-r")) {
        numReducer = Integer.parseInt(args[++i]);
        if (numReducer < 0) {
          printUsage(numReducer + ": numReducer must be >= 0");
        }
      }
      else if(args[i].equals("-mt")) {
        mapSleepTime = Long.parseLong(args[++i]);
        if (mapSleepTime < 0) {
          printUsage(mapSleepTime + ": mapSleepTime must be >= 0");
        }
      }
        ...
        if (recSleepTime < 0) {
          printUsage(recSleepTime + ": recordSleepTime must be >= 0");
        }
      }
    ...
    for (int i = 0; i < deadlines.length; i++) {
        Job job = createJob(numMapper, numReducer, mapSleepTime,
                    mapSleepCount, reduceSleepTime, reduceSleepCount, deadlines[i]);
        jobs.add(job);
        job.submit();
    }
