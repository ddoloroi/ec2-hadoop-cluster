...
import org.apache.hadoop.yarn.server.resourcemanager.webapp.dao.DeadlineSchedulerInfo;
import org.apache.hadoop.yarn.server.resourcemanager.webapp.dao.DeadlineSchedulerLeafQueueInfo;
import org.apache.hadoop.yarn.server.resourcemanager.webapp.dao.DeadlineSchedulerQueueInfo;
...
public class DeadlineSchedulerPage extends RmView {
  ...
  static class FSQInfo {
    DeadlineSchedulerQueueInfo qinfo;
  }

  static class LeafQueueBlock extends HtmlBlock {
    final DeadlineSchedulerLeafQueueInfo qinfo;

    @Inject LeafQueueBlock(ViewContext ctx, FSQInfo info) {
      super(ctx);
      qinfo = (DeadlineSchedulerLeafQueueInfo)info.qinfo;
    }
  ...
  static class QueueBlock extends HtmlBlock {
    ...
    public void render(Block html) {
      Collection<DeadlineSchedulerQueueInfo> subQueues = fsqinfo.qinfo.getChildQueues();
      UL<Hamlet> ul = html.ul("#pq");
      for (DeadlineSchedulerQueueInfo info : subQueues) {
        ...
        fsqinfo.qinfo = info;
        if (info instanceof DeadlineSchedulerLeafQueueInfo) {
  ...
  static class QueuesBlock extends HtmlBlock {
    final DeadlineScheduler fs;
    final FSQInfo fsqinfo;

    @Inject QueuesBlock(ResourceManager rm, FSQInfo info) {
      fs = (DeadlineScheduler)rm.getResourceScheduler();
      fsqinfo = info;
    }
    ...
    public void render(Block html) {
      ...
      if (fs == null) {
        ...
        DeadlineSchedulerInfo sinfo = new DeadlineSchedulerInfo(fs);
      ...
      script().$type("text/javascript").
          _("$('#cs').hide();")._()._().
      _(DeadlineSchedulerAppsBlock.class);
    }
  }
