 ...
 */
public class DeadlineSchedulerAppsBlock extends HtmlBlock {
  final ConcurrentMap<ApplicationId, RMApp> apps;
  final DeadlineSchedulerInfo fsinfo;
  final Configuration conf;
  final ResourceManager rm;
  @Inject
  public DeadlineSchedulerAppsBlock(ResourceManager rm, ViewContext ctx,
      Configuration conf) {
    super(ctx);
    DeadlineScheduler scheduler = (DeadlineScheduler) rm.getResourceScheduler();
    fsinfo = new DeadlineSchedulerInfo(scheduler);
  ...
  @Override public void render(Block html) {
    ...
    for (RMApp app : apps.values()) {
      ...
      if (fairShare == DeadlineSchedulerInfo.INVALID_FAIR_SHARE) {
        // DeadlineScheduler#applications don't have the entry. Skip it.
        continue;
      }
