...
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.deadline.AllocationConfiguration;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.deadline.FSLeafQueue;
...
@XmlSeeAlso({DeadlineSchedulerLeafQueueInfo.class})
public class DeadlineSchedulerQueueInfo {
  private int maxApps;
  ...
  private Collection<DeadlineSchedulerQueueInfo> childQueues;

  public DeadlineSchedulerQueueInfo() {
  }

  public DeadlineSchedulerQueueInfo(FSQueue queue, DeadlineScheduler scheduler) {
    ...
    childQueues = new ArrayList<DeadlineSchedulerQueueInfo>();
    ...
    for (FSQueue child : children) {
      if (child instanceof FSLeafQueue) {
        childQueues.add(new DeadlineSchedulerLeafQueueInfo((FSLeafQueue)child, scheduler));
      } else {
        childQueues.add(new DeadlineSchedulerQueueInfo(child, scheduler));
      }
    }
  }
  ...
  public Collection<DeadlineSchedulerQueueInfo> getChildQueues() {
    return childQueues;
  }
}
